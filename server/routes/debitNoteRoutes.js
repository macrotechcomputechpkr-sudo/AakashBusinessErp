...
// A Debit Note reduces what we owe a vendor (or increases a receivable)
// - pure ledger lines, no products. Reuses the SAME bill-wise
// settlement engine Purchase Return already uses (a DN is a 'dr'
// reference, settling outstanding 'cr' vendor bills FIFO), and posts
// to the same shared ledger_transaction_batches/lines table every
// other posted document uses.
// =============================================
...
async function syncDetails(tenantClient, tenantId, dnId, details) {
    await tenantClient.from('debit_note_details').delete().eq('debit_note_id', dnId);
    ...
    const rows = await Promise.all(details.map(async (d, i) => {
        ...
        return { tenant_id: tenantId, debit_note_id: dnId, display_order: i + 1, ledger_id: d.ledger_id, sub_ledger_id: d.sub_ledger_id || null, product_company_id: d.product_company_id || null, amount: Number(d.amount) || 0, narration: d.narration || null, ledger_name_snapshot: ledger?.account_name || null };
    }));
    const { error } = await tenantClient.from('debit_note_details').insert(rows);
...
async function postDnToLedger(tenantClient, tenantId, dn, details, userId) {
    const { data: batch, error } = await tenantClient
        .from('ledger_transaction_batches')
        .insert({ tenant_id: tenantId, document_type: 'debit_note', document_id: dn.id, batch_date: dn.doc_date, narration: dn.narration || `Debit Note ${dn.doc_no}`, created_by: userId })
        .select().single();
    if (error) throw error;
    const rows = [
        ...
        ...partyLinesByCompany(details, dn.total_amount).map(([companyId, amt]) => ({ tenant_id: tenantId, batch_id: batch.id, ledger_account_id: dn.party_ledger_id, sub_ledger_id: dn.party_sub_ledger_id || null, product_company_id: companyId, debit_amount: amt, credit_amount: 0, narration: dn.narration })),
        ...details.map(d => ({ tenant_id: tenantId, batch_id: batch.id, ledger_account_id: d.ledger_id, sub_ledger_id: d.sub_ledger_id || null, product_company_id: d.product_company_id || null, debit_amount: 0, credit_amount: d.amount, narration: d.narration || dn.narration }))
    ];
    const { error: lineErr } = await tenantClient.from('ledger_transaction_lines').insert(rows);
    if (lineErr) throw lineErr;
}

async function reverseDnLedgerBatch(tenantClient, dnId) {
    const { data: batches } = await tenantClient.from('ledger_transaction_batches').select('id').eq('document_type', 'debit_note').eq('document_id', dnId);
...
router.get('/debit-notes', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient.from('debit_notes').select('*').eq('tenant_id', req.auth.tenantId).order('doc_date', { ascending: false });
        if (error) throw error;
...
router.get('/debit-notes/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient.from('debit_notes').select('*').eq('id', req.params.id).eq('tenant_id', req.auth.tenantId).single();
        if (error) return res.status(404).json({ success: false, error: 'Debit Note not found' });
        const { data: details } = await tenantClient.from('debit_note_details').select('*').eq('debit_note_id', req.params.id).order('display_order');
...
router.post('/debit-notes', requireAuth, loadUserPermissions, requirePermission('ledger', 'create'), async (req, res) => {
    try {
        ...
        const fieldError = await checkCompulsoryFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'debit_note', req.body, isDraft);
        ...
        try {
            docNo = await resolveDocumentNumber(tenantClient, {
                tenantId, voucherType: 'debit_note', userId: req.auth.userId,
                categoryId: b.numbering_category_id, manualNumber: b.doc_no, tableName: 'debit_notes',
        ...
        if (!docNo) {
            const { data: codeRow, error: codeErr } = await tenantClient.rpc('next_master_code', { seq_name: 'tenant_master.seq_debit_note_code', type_prefix: 'DN' });
        ...
        try {
            ...
            await tenantClient.from('debit_notes').update({ total_amount: total }).eq('id', doc.id);
        } catch (syncErr) {
            await tenantClient.from('debit_note_details').delete().eq('debit_note_id', doc.id);
            await tenantClient.from('debit_notes').delete().eq('id', doc.id);
            return res.status(400).json({ success: false, error: syncErr.message || 'Could not save lines' });
        }

        await logAudit(tenantId, req.auth.userId, 'create_debit_note', 'debit_note', doc.id, { doc_no: doc.doc_no });
        await logDocumentAudit(tenantClient, tenantId, 'debit_note', doc.id, 'create', req.auth.userId);
        res.json({ success: true, message: `Debit Note ${doc.doc_no} created`, data: doc });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

router.put('/debit-notes/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'edit'), async (req, res) => {
    try {
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const b = req.body;
        const { data: existing } = await tenantClient.from('debit_notes').select('*').eq('id', req.params.id).eq('tenant_id', tenantId).single();
        ...
        if (['posted', 'cancelled'].includes(existing.status)) {
            return res.status(400).json({ success: false, error: `Cannot edit a ${existing.status} debit note` });
        }

        // Readonly / disabled header fields keep their stored value (before snapshots + update).

        await lockProtectedFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'debit_note', b, existing);

        const snapshots = await captureMasterSnapshots(tenantClient, { ...existing, ...b });
        const fieldError = await checkCompulsoryFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'debit_note', b, !!(b.save_as_draft || b.status === 'draft'));
        if (fieldError) return res.status(400).json({ success: false, error: fieldError });
        ...
        const { data, error } = await tenantClient.from('debit_notes').update(update).eq('id', req.params.id).eq('tenant_id', tenantId).select().single();
        if (error) throw error;

        if (b.details) {
            const total = await syncDetails(tenantClient, tenantId, req.params.id, b.details);
            await tenantClient.from('debit_notes').update({ total_amount: total }).eq('id', req.params.id);
        }

        await logAudit(tenantId, req.auth.userId, 'update_debit_note', 'debit_note', req.params.id, { old_data: existing, new_data: data });
        await logDocumentAudit(tenantClient, tenantId, 'debit_note', req.params.id, 'update', req.auth.userId);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
router.put('/debit-notes/:id/status', requireAuth, loadUserPermissions, requirePermission('ledger', 'edit'), async (req, res) => {
    try {
        ...
        if (status === 'cancelled' && existing.status === 'posted') {
            const blockMsg = await checkCanCancelIfSettled(tenantClient, tenantId, 'debit_note', req.params.id);
        ...
        const { data: details } = await tenantClient.from('debit_note_details').select('*').eq('debit_note_id', req.params.id);
        ...
        if (status === 'posted' && existing.status !== 'posted') {
            await postDnToLedger(tenantClient, tenantId, data, details || [], req.auth.userId);
            ...
            if (data.party_ledger_id) {
                ...
                if (bwEnabled) {
                    ...
                    if (!settlements) {
                        const outstanding = await getOutstandingReferences(tenantClient, data.party_ledger_id, 'cr', noteCompany);
                    ...
                    await createReferenceAndSettle(tenantClient, tenantId, { productCompanyId: noteCompany,
                        ledgerId: data.party_ledger_id, sourceType: 'debit_note', sourceId: data.id,
            ...
            await reverseDnLedgerBatch(tenantClient, req.params.id);
            await reverseReferenceAndSettlements(tenantClient, 'debit_note', req.params.id);
        }

        await logAudit(tenantId, req.auth.userId, 'change_debit_note_status', 'debit_note', req.params.id, { new_status: status, cancellation_reason });
        await logDocumentAudit(tenantClient, tenantId, 'debit_note', req.params.id, 'status_change', req.auth.userId);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
router.delete('/debit-notes/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'delete'), async (req, res) => {
    try {
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const { data: existing } = await tenantClient.from('debit_notes').select('*').eq('id', req.params.id).eq('tenant_id', tenantId).single();
        if (!existing) return res.status(404).json({ success: false, error: 'Debit Note not found' });
        if (existing.status !== 'draft') return res.status(400).json({ success: false, error: 'Only a Draft can be deleted - use Cancel for a posted debit note' });
        const { error } = await tenantClient.from('debit_notes').delete().eq('id', req.params.id).eq('tenant_id', tenantId);
        if (error) throw error;
        await logAudit(tenantId, req.auth.userId, 'delete_draft_debit_note', 'debit_note', req.params.id, { old_data: existing });
...
router.get('/debit-notes/:id/audit-trail', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient
            .from('document_audit_trail').select('*, performer:performed_by(full_name)')
            .eq('tenant_id', req.auth.tenantId).eq('document_type', 'debit_note').eq('document_id', req.params.id)
