// =============================================
// routes/purchaseQuotationRoutes.js
// Full CRUD for Purchase Quotation - can pull forward from a Requisition
...
// FEATURE: "Save as Draft" - a draft only needs a Date; everything else
// (Vendor, line items) can be filled in later. A non-draft ("final")
// save still needs the full, real validation.
...
async function syncDetails(tenantClient, tenantId, quotationId, details) {
    await tenantClient.from('purchase_quotation_details').delete().eq('quotation_id', quotationId);
    ...
    const { data: insertedRows, error } = await tenantClient.from('purchase_quotation_details').insert(rows).select('id, display_order').order('display_order');
...
// every line into ready-to-use Quotation detail rows, carrying the source
// ids forward so GRN can later track received-vs-ordered progress.
router.get('/purchase-quotations/pull-forward', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const { requisition_id } = req.query;
        if (!requisition_id) return res.status(400).json({ success: false, error: 'Provide a requisition_id to pull from' });
...
router.get('/purchase-quotations', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        await ensureDefaultFieldControls(tenantClient, req.auth.tenantId, 'purchase_quotation');
...
router.get('/purchase-quotations/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient
            .from('purchase_quotations')
            .select('*, details:purchase_quotation_details(*)')
            .eq('id', req.params.id).eq('tenant_id', req.auth.tenantId).single();
        if (error) return res.status(404).json({ success: false, error: 'Purchase Quotation not found' });

        const { data: appliedTerms } = await tenantClient
            .from('document_billing_terms').select('billing_term_id').eq('document_type', 'purchase_quotation').eq('document_id', req.params.id);
...
router.post('/purchase-quotations', requireAuth, loadUserPermissions, requirePermission('ledger', 'create'), async (req, res) => {
    try {
        ...
        const fieldError = await checkCompulsoryFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'purchase_quotation', req.body, isDraft);
        ...
        try {
            docNo = await resolveDocumentNumber(tenantClient, {
                tenantId, voucherType: 'purchase_quotation', userId: req.auth.userId,
                categoryId: b.numbering_category_id, manualNumber: b.doc_no, tableName: 'purchase_quotations',
        ...
        if (!docNo) {
            const { data: codeRow, error: codeErr } = await tenantClient.rpc('next_master_code', { seq_name: 'tenant_master.seq_purchase_quotation_code', type_prefix: 'PQUO' });
        ...
        try {
            ...
            const documentAdjustment = await syncBillingTerms(tenantClient, tenantId, 'purchase_quotation', doc.id, b.billing_term_ids, total, totalQty);
            const lineTermResult = await syncLineBillingTerms(tenantClient, tenantId, 'purchase_quotation', doc.id, detailsToSave, detailIdByIndex, b.summary_overrides);
            await applyTermSubLedgers(tenantClient, tenantId, 'purchase_quotation', doc.id, b.term_sub_ledgers);
            await tenantClient.from('purchase_quotations').update({ total_amount: total + documentAdjustment + lineTermResult.total }).eq('id', doc.id);

            // FEATURE: keeps the source Requisition's qty_quoted current,
            // so its own Outstanding Report reflects how much has
            // actually been quoted so far.
            for (const d of detailsToSave) {
                if (d.source_requisition_detail_id) {
                    const { data: srcLine } = await tenantClient.from('purchase_requisition_details').select('qty_quoted').eq('id', d.source_requisition_detail_id).maybeSingle();
                    if (srcLine) {
                        await tenantClient.from('purchase_requisition_details').update({ qty_quoted: Number(srcLine.qty_quoted || 0) + Number(d.qty) }).eq('id', d.source_requisition_detail_id);
                    }
                }
            }
        } catch (syncErr) {
            await tenantClient.from('document_line_billing_terms').delete().eq('document_type', 'purchase_quotation').eq('document_id', doc.id);
            await tenantClient.from('document_billing_terms').delete().eq('document_type', 'purchase_quotation').eq('document_id', doc.id);
            await tenantClient.from('purchase_quotations').delete().eq('id', doc.id);
            return res.status(400).json({ success: false, error: syncErr.message || 'Could not save line items' });
        }

        await logAudit(tenantId, req.auth.userId, 'create_purchase_quotation', 'purchase_quotation', doc.id, { doc_no: doc.doc_no });
        await logDocumentAudit(tenantClient, tenantId, 'purchase_quotation', doc.id, 'create', req.auth.userId);
        res.json({ success: true, message: `Quotation ${doc.doc_no} created`, data: doc });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

router.put('/purchase-quotations/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'edit'), async (req, res) => {
    try {
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const b = req.body;
        const { data: existing } = await tenantClient.from('purchase_quotations').select('*').eq('id', req.params.id).eq('tenant_id', tenantId).single();
        if (!existing) return res.status(404).json({ success: false, error: 'Purchase Quotation not found' });
        if (['accepted', 'rejected', 'cancelled'].includes(existing.status)) {
            return res.status(400).json({ success: false, error: `Cannot edit a ${existing.status} quotation` });
        }
        ...
        await lockProtectedFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'purchase_quotation', b, existing);
        ...
        const fieldError = await checkCompulsoryFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'purchase_quotation', b, !!(b.save_as_draft || b.status === 'draft'));
        ...
        const { data, error } = await tenantClient.from('purchase_quotations').update(update).eq('id', req.params.id).eq('tenant_id', tenantId).select().single();
        if (error) throw error;
        ...
        if (b.details) {
            ...
            const documentAdjustment = await syncBillingTerms(tenantClient, tenantId, 'purchase_quotation', req.params.id, b.billing_term_ids, total, totalQty);
            const lineTermResult = await syncLineBillingTerms(tenantClient, tenantId, 'purchase_quotation', req.params.id, b.details, detailIdByIndex, b.summary_overrides);
            await applyTermSubLedgers(tenantClient, tenantId, 'purchase_quotation', req.params.id, b.term_sub_ledgers);
            await tenantClient.from('purchase_quotations').update({ total_amount: total + documentAdjustment + lineTermResult.total }).eq('id', req.params.id);
        }
        ...
        await logDocumentAudit(tenantClient, tenantId, 'purchase_quotation', req.params.id, 'update', req.auth.userId, changes);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
router.put('/purchase-quotations/:id/status', requireAuth, loadUserPermissions, requirePermission('ledger', 'edit'), async (req, res) => {
    try {
        const { status } = req.body;
        if (!['draft', 'sent', 'received', 'accepted', 'rejected', 'cancelled'].includes(status)) {
            return res.status(400).json({ success: false, error: 'Invalid status' });
        }
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const { data, error } = await tenantClient.from('purchase_quotations').update({ status, updated_by: req.auth.userId }).eq('id', req.params.id).eq('tenant_id', tenantId).select().single();
        if (error) throw error;
        await logAudit(tenantId, req.auth.userId, 'change_quotation_status', 'purchase_quotation', req.params.id, { new_status: status });
        await logDocumentAudit(tenantClient, tenantId, 'purchase_quotation', req.params.id, 'status_change', req.auth.userId, [{ field_key: 'status', new_value: status }]);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
router.get('/purchase-quotations/:id/audit-trail', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient
            .from('document_audit_trail')
            .select('*, performer:performed_by(full_name)')
            .eq('tenant_id', req.auth.tenantId).eq('document_type', 'purchase_quotation').eq('document_id', req.params.id)
...
// document still sitting in 'draft' status.
router.delete('/purchase-quotations/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'delete'), async (req, res) => {
    try {
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const { data: existing } = await tenantClient.from('purchase_quotations').select('*').eq('id', req.params.id).eq('tenant_id', tenantId).single();
        ...
        if (existing.status !== 'draft') {
            return res.status(400).json({ success: false, error: 'Only a Draft can be deleted - use Cancel for a sent/received quotation' });
        }
        await tenantClient.from('document_line_billing_terms').delete().eq('document_type', 'purchase_quotation').eq('document_id', req.params.id);
        await tenantClient.from('document_billing_terms').delete().eq('document_type', 'purchase_quotation').eq('document_id', req.params.id);
        const { error } = await tenantClient.from('purchase_quotations').delete().eq('id', req.params.id).eq('tenant_id', tenantId);
        if (error) throw error;
        await logAudit(tenantId, req.auth.userId, 'delete_draft_purchase_quotation', 'purchase_quotation', req.params.id, { old_data: existing });
