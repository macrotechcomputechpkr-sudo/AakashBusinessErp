// =============================================
// routes/salesNonsaleableReturnRoutes.js
// Customer returns damaged/expired goods - tracked in the SAME separate
// nonsaleable_stock_movements ledger built for Purchase (file 65),
// never the main stock. 'credit_note' settlement credits the customer
// (a 'cr' bill-wise reference, like a regular Sales Return); 'no_credit'
// is pure stock/record-keeping with zero GL impact.
// =============================================
...
async function captureMasterSnapshots(tenantClient, b) {
    ...
    const [customer, subLedger, warehouse, costCenter, businessUnit] = await Promise.all(lookups.map(l => l || Promise.resolve({ data: null })));
    return {
...
async function syncDetails(tenantClient, tenantId, returnId, details) {
    await tenantClient.from('sales_nonsaleable_return_details').delete().eq('return_id', returnId);
    ...
    const rows = await Promise.all(details.map(async (d, i) => {
        ...
        let qty = Number(d.qty) || 0, rate = Number(d.rate) || 0, gross;
        ...
        return {
            ...
            rate, discount_percent: d.discount_percent || 0, discount_amount: discountAmount, amount: gross - discountAmount,
            warehouse_id: d.warehouse_id || null, batch_no: d.batch_no || null,
            ...snapshots
        };
    }));
    const { error } = await tenantClient.from('sales_nonsaleable_return_details').insert(rows);
...
async function postNonsaleableStockMovements(tenantClient, tenantId, returnDoc, details) {
    ...
    for (const d of details) {
        ...
        rows.push({ tenant_id: tenantId, product_id: d.product_id, warehouse_id: wh, batch_no: d.batch_no, movement_date: returnDoc.doc_date, qty_in: baseQty, qty_out: 0, unit_cost: unitCost, source_type: 'sales_nonsalable_return', source_id: returnDoc.id, source_detail_id: d.id, narration: `Sales Non-saleable Return ${returnDoc.doc_no}` });
    }
    if (rows.length > 0) {
...
async function reverseNonsaleableStockMovements(tenantClient, returnId) {
    await tenantClient.from('nonsaleable_stock_movements').delete().eq('source_type', 'sales_nonsalable_return').eq('source_id', returnId);
}

async function postReturnToLedger(tenantClient, tenantId, returnDoc, userId) {
    if (returnDoc.settlement_type !== 'credit_note') return;
    const { data: batch, error } = await tenantClient
        .from('ledger_transaction_batches')
        .insert({ tenant_id: tenantId, document_type: 'sales_nonsalable_return', document_id: returnDoc.id, batch_date: returnDoc.doc_date, narration: returnDoc.narration || `Sales Non-saleable Return ${returnDoc.doc_no}`, created_by: userId })
        .select().single();
    if (error) throw error;
...
async function reverseReturnGlBatch(tenantClient, returnId) {
    const { data: batches } = await tenantClient.from('ledger_transaction_batches').select('id').eq('document_type', 'sales_nonsalable_return').eq('document_id', returnId);
...
router.get('/sales-nonsaleable-returns', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient.from('sales_nonsaleable_returns').select('*').eq('tenant_id', req.auth.tenantId).order('doc_date', { ascending: false });
        if (error) throw error;
...
router.get('/sales-nonsaleable-returns/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient.from('sales_nonsaleable_returns').select('*').eq('id', req.params.id).eq('tenant_id', req.auth.tenantId).single();
        if (error) return res.status(404).json({ success: false, error: 'Sales Non-saleable Return not found' });
        const { data: details } = await tenantClient.from('sales_nonsaleable_return_details').select('*').eq('return_id', req.params.id).order('display_order');
...
router.post('/sales-nonsaleable-returns', requireAuth, loadUserPermissions, requirePermission('ledger', 'create'), async (req, res) => {
    try {
        ...
        if (!docNo) {
            const { data: codeRow, error: codeErr } = await tenantClient.rpc('next_master_code', { seq_name: 'tenant_master.seq_sales_nsreturn_code', type_prefix: 'SNSR' });
        ...
        const { data: doc, error } = await tenantClient
            .from('sales_nonsaleable_returns')
            .insert({
        ...
        try {
            ...
            await tenantClient.from('sales_nonsaleable_returns').update({ total_amount: total }).eq('id', doc.id);
        } catch (syncErr) {
            await tenantClient.from('sales_nonsaleable_return_details').delete().eq('return_id', doc.id);
            await tenantClient.from('sales_nonsaleable_returns').delete().eq('id', doc.id);
            return res.status(400).json({ success: false, error: syncErr.message || 'Could not save return lines' });
        }

        await logAudit(tenantId, req.auth.userId, 'create_sales_nonsaleable_return', 'sales_nonsalable_return', doc.id, { doc_no: doc.doc_no });
        await logDocumentAudit(tenantClient, tenantId, 'sales_nonsalable_return', doc.id, 'create', req.auth.userId);
        res.json({ success: true, message: `Sales Non-saleable Return ${doc.doc_no} created`, data: doc });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

router.put('/sales-nonsaleable-returns/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'edit'), async (req, res) => {
    try {
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const b = req.body;
        const { data: existing } = await tenantClient.from('sales_nonsaleable_returns').select('*').eq('id', req.params.id).eq('tenant_id', tenantId).single();
        if (!existing) return res.status(404).json({ success: false, error: 'Sales Non-saleable Return not found' });
        ...
        await lockProtectedFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'sales_nonsalable_return', b, existing);
        ...
        const { data, error } = await tenantClient.from('sales_nonsaleable_returns').update(update).eq('id', req.params.id).eq('tenant_id', tenantId).select().single();
        if (error) throw error;

        if (b.details) {
            const total = await syncDetails(tenantClient, tenantId, req.params.id, b.details);
            await tenantClient.from('sales_nonsaleable_returns').update({ total_amount: total }).eq('id', req.params.id);
        }

        await logAudit(tenantId, req.auth.userId, 'update_sales_nonsaleable_return', 'sales_nonsalable_return', req.params.id, { old_data: existing, new_data: data });
        await logDocumentAudit(tenantClient, tenantId, 'sales_nonsalable_return', req.params.id, 'update', req.auth.userId);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
router.put('/sales-nonsaleable-returns/:id/status', requireAuth, loadUserPermissions, requirePermission('ledger', 'edit'), async (req, res) => {
    try {
        ...
        if (status === 'cancelled' && existing.status === 'posted') {
            const blockMsg = await checkCanCancelIfSettled(tenantClient, tenantId, 'sales_nonsalable_return', req.params.id);
        ...
        const { data: returnDetails } = await tenantClient.from('sales_nonsaleable_return_details').select('*').eq('return_id', req.params.id);
        ...
        if (status === 'posted' && existing.status !== 'posted') {
            ...
            if (data.settlement_type === 'credit_note' && data.customer_ledger_id) {
                ...
                if (bwEnabled) {
                    ...
                    await createReferenceAndSettle(tenantClient, tenantId, { productCompanyId: data.product_company_id || null,
                        ledgerId: data.customer_ledger_id, sourceType: 'sales_nonsalable_return', sourceId: data.id,
            ...
            await reverseReferenceAndSettlements(tenantClient, 'sales_nonsalable_return', req.params.id);
        }

        await logAudit(tenantId, req.auth.userId, 'change_sales_nonsaleable_return_status', 'sales_nonsalable_return', req.params.id, { new_status: status, cancellation_reason });
        await logDocumentAudit(tenantClient, tenantId, 'sales_nonsalable_return', req.params.id, 'status_change', req.auth.userId);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
router.delete('/sales-nonsaleable-returns/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'delete'), async (req, res) => {
    try {
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const { data: existing } = await tenantClient.from('sales_nonsaleable_returns').select('*').eq('id', req.params.id).eq('tenant_id', tenantId).single();
        ...
        const { error } = await tenantClient.from('sales_nonsaleable_returns').delete().eq('id', req.params.id).eq('tenant_id', tenantId);
        if (error) throw error;
        await logAudit(tenantId, req.auth.userId, 'delete_draft_sales_nonsaleable_return', 'sales_nonsalable_return', req.params.id, { old_data: existing });
...
router.get('/sales-nonsaleable-returns/:id/audit-trail', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient
            .from('document_audit_trail').select('*, performer:performed_by(full_name)')
            .eq('tenant_id', req.auth.tenantId).eq('document_type', 'sales_nonsalable_return').eq('document_id', req.params.id)
