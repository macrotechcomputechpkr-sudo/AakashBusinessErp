// =============================================
// routes/purchaseNonsaleableReturnRoutes.js
// Full CRUD for Purchase Non-saleable Return - links to Bill only
...
async function validateReturnQuantities(tenantClient, details) {
...
async function syncDetails(tenantClient, tenantId, returnId, details) {
    await tenantClient.from('purchase_nonsaleable_return_details').delete().eq('return_id', returnId);
    ...
    const { data: insertedRows, error } = await tenantClient.from('purchase_nonsaleable_return_details').insert(rows).select('id, display_order').order('display_order');
...
router.get('/purchase-nonsaleable-returns/pull-forward', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
...
router.get('/purchase-nonsaleable-returns', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        await ensureDefaultFieldControls(tenantClient, req.auth.tenantId, 'purchase_nonsalable_return');
        const { data, error } = await tenantClient
            .from('purchase_nonsaleable_returns')
...
router.get('/purchase-nonsaleable-returns/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient
            .from('purchase_nonsaleable_returns')
        ...
        if (error) return res.status(404).json({ success: false, error: 'Non-saleable Return not found' });

        const { data: appliedTerms } = await tenantClient
            .from('document_billing_terms').select('billing_term_id').eq('document_type', 'purchase_nonsalable_return').eq('document_id', req.params.id);
        data.billing_term_ids = (appliedTerms || []).map(t => t.billing_term_id);

        const { data: lineTerms } = await tenantClient
            .from('document_line_billing_terms').select('detail_id, billing_term_id').eq('document_type', 'purchase_nonsalable_return').eq('document_id', req.params.id);
        if (data.details) {
...
router.post('/purchase-nonsaleable-returns', requireAuth, loadUserPermissions, requirePermission('ledger', 'create'), async (req, res) => {
    try {
        ...
        if (!docNo) {
            const { data: codeRow, error: codeErr } = await tenantClient.rpc('next_master_code', { seq_name: 'tenant_master.seq_purchase_nonsaleable_return_code', type_prefix: 'PNSR' });
        ...
        const { data: doc, error } = await tenantClient
            ...
            .insert({
                ...
                return_reason: b.return_reason || 'damaged', settlement_type: b.settlement_type || 'credit_note', narration: b.narration || null,
        ...
        try {
            ...
            const documentAdjustment = await syncBillingTerms(tenantClient, tenantId, 'purchase_nonsalable_return', doc.id, b.billing_term_ids, total, totalQty);
            const lineTermResult = await syncLineBillingTerms(tenantClient, tenantId, 'purchase_nonsalable_return', doc.id, detailsToSave, detailIdByIndex, b.summary_overrides);
            await applyTermSubLedgers(tenantClient, tenantId, 'purchase_nonsalable_return', doc.id, b.term_sub_ledgers);
            await tenantClient.from('purchase_nonsaleable_returns').update({ total_amount: total + documentAdjustment + lineTermResult.total }).eq('id', doc.id);
        } catch (syncErr) {
            await tenantClient.from('document_line_billing_terms').delete().eq('document_type', 'purchase_nonsalable_return').eq('document_id', doc.id);
            await tenantClient.from('document_billing_terms').delete().eq('document_type', 'purchase_nonsalable_return').eq('document_id', doc.id);
            await tenantClient.from('purchase_nonsaleable_returns').delete().eq('id', doc.id);
            return res.status(400).json({ success: false, error: syncErr.message || 'Could not save line items' });
        }

        await logAudit(tenantId, req.auth.userId, 'create_nonsaleable_return', 'purchase_nonsalable_return', doc.id, { doc_no: doc.doc_no });
        await logDocumentAudit(tenantClient, tenantId, 'purchase_nonsalable_return', doc.id, 'create', req.auth.userId);
        res.json({ success: true, message: `Non-saleable Return ${doc.doc_no} created`, data: doc });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

router.put('/purchase-nonsaleable-returns/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'edit'), async (req, res) => {
    try {
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const b = req.body;
        const { data: existing } = await tenantClient.from('purchase_nonsaleable_returns').select('*').eq('id', req.params.id).eq('tenant_id', tenantId).single();
        if (!existing) return res.status(404).json({ success: false, error: 'Non-saleable Return not found' });
        ...
        await lockProtectedFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'purchase_nonsalable_return', b, existing);
        ...
        const { data, error } = await tenantClient.from('purchase_nonsaleable_returns').update(update).eq('id', req.params.id).eq('tenant_id', tenantId).select().single();
        if (error) throw error;
        ...
        if (b.details) {
            ...
            const documentAdjustment = await syncBillingTerms(tenantClient, tenantId, 'purchase_nonsalable_return', req.params.id, b.billing_term_ids, total, totalQty);
            const lineTermResult = await syncLineBillingTerms(tenantClient, tenantId, 'purchase_nonsalable_return', req.params.id, b.details, detailIdByIndex, b.summary_overrides);
            await applyTermSubLedgers(tenantClient, tenantId, 'purchase_nonsalable_return', req.params.id, b.term_sub_ledgers);
            await tenantClient.from('purchase_nonsaleable_returns').update({ total_amount: total + documentAdjustment + lineTermResult.total }).eq('id', req.params.id);
        }

        await logAudit(tenantId, req.auth.userId, 'update_nonsaleable_return', 'purchase_nonsalable_return', req.params.id, { old_data: existing, new_data: data });
        ...
        await logDocumentAudit(tenantClient, tenantId, 'purchase_nonsalable_return', req.params.id, 'update', req.auth.userId, changes);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
async function postNonsaleableStockMovements(tenantClient, tenantId, nonsaleableReturn, details) {
    ...
    for (const d of details) {
        ...
        rows.push({ tenant_id: tenantId, product_id: d.product_id, warehouse_id: d.warehouse_id, batch_no: d.batch_no, movement_date: nonsaleableReturn.doc_date, qty_in: baseQty, qty_out: 0, unit_cost: unitCost, source_type: 'purchase_nonsalable_return', source_id: nonsaleableReturn.id, source_detail_id: d.id, narration: `Non-saleable Return ${nonsaleableReturn.doc_no}` });
    }
    if (rows.length > 0) {
        const { error } = await tenantClient.from('nonsaleable_stock_movements').insert(rows);
        if (error) throw error;
    }
}

async function reverseNonsaleableStockMovements(tenantClient, returnId) {
    await tenantClient.from('nonsaleable_stock_movements').delete().eq('source_type', 'purchase_nonsalable_return').eq('source_id', returnId);
}

// FEATURE: "Valuation le main stock Account ma Effect Garne Hoena,
// Income Statement ra Balance Sheet ma matra effect garne ho" - a
// write-off is a genuine loss (Dr Write-off Expense - hits the P&L, Cr
// Non-saleable Stock Asset - leaves the Balance Sheet); a credit-note
// settlement instead reduces what we owe the vendor, exactly like a
// regular Purchase Return, and reuses the SAME bill-wise settlement
// engine.
async function postNonsaleableReturnToLedger(tenantClient, tenantId, nonsaleableReturn, userId) {
    const { data: sysControl } = await tenantClient.from('system_control_settings').select('nonsaleable_writeoff_expense_ledger_id, nonsaleable_stock_asset_ledger_id').eq('tenant_id', tenantId).maybeSingle();
    if (nonsaleableReturn.settlement_type === 'write_off') {
        if (!sysControl?.nonsaleable_writeoff_expense_ledger_id || !sysControl?.nonsaleable_stock_asset_ledger_id) return;
        const { data: batch, error } = await tenantClient
            .from('ledger_transaction_batches')
            .insert({ tenant_id: tenantId, document_type: 'purchase_nonsalable_return', document_id: nonsaleableReturn.id, batch_date: nonsaleableReturn.doc_date, narration: nonsaleableReturn.narration || `Non-saleable Return ${nonsaleableReturn.doc_no} - written off`, created_by: userId })
            .select().single();
        if (error) throw error;
        const rows = [
            { tenant_id: tenantId, batch_id: batch.id, ledger_account_id: sysControl.nonsaleable_writeoff_expense_ledger_id, debit_amount: nonsaleableReturn.total_amount, credit_amount: 0 },
            { tenant_id: tenantId, batch_id: batch.id, ledger_account_id: sysControl.nonsaleable_stock_asset_ledger_id, debit_amount: 0, credit_amount: nonsaleableReturn.total_amount }
        ];
        const { error: lineErr } = await tenantClient.from('ledger_transaction_lines').insert(rows);
        if (lineErr) throw lineErr;
    } else if (nonsaleableReturn.settlement_type === 'credit_note' && nonsaleableReturn.vendor_ledger_id) {
        const bwEnabled = await isBillWiseTrackingEnabled(tenantClient, tenantId, nonsaleableReturn.vendor_ledger_id);
        if (!bwEnabled) return;
        const outstanding = await getOutstandingReferences(tenantClient, nonsaleableReturn.vendor_ledger_id, 'cr', nonsaleableReturn.product_company_id || null);
        const settlements = computeFifoAllocation(outstanding, nonsaleableReturn.total_amount).allocations;
        await createReferenceAndSettle(tenantClient, tenantId, { productCompanyId: nonsaleableReturn.product_company_id || null,
            ledgerId: nonsaleableReturn.vendor_ledger_id, sourceType: 'purchase_nonsalable_return', sourceId: nonsaleableReturn.id,
            docNo: nonsaleableReturn.doc_no, date: nonsaleableReturn.doc_date, nature: 'dr', totalAmount: nonsaleableReturn.total_amount, settlements
        });
    }
}

async function reverseNonsaleableReturnLedger(tenantClient, returnId) {
    const { data: batches } = await tenantClient.from('ledger_transaction_batches').select('id').eq('document_type', 'purchase_nonsalable_return').eq('document_id', returnId);
    ...
    await reverseReferenceAndSettlements(tenantClient, 'purchase_nonsalable_return', returnId);
}

router.put('/purchase-nonsaleable-returns/:id/status', requireAuth, loadUserPermissions, requirePermission('ledger', 'edit'), async (req, res) => {
    try {
        ...
        const { data: returnDetails } = await tenantClient.from('purchase_nonsaleable_return_details').select('*').eq('return_id', req.params.id);
        if (status === 'posted' && existing.status !== 'posted') {
            ...
            // FEATURE: "xuttai rakhne" - a SEPARATE stock ledger, never
            // the main stock_movements one, plus the settlement-type-
            // appropriate GL entry.
            await postNonsaleableStockMovements(tenantClient, tenantId, data, returnDetails || []);
            await postNonsaleableReturnToLedger(tenantClient, tenantId, data, req.auth.userId);
        } else if (status === 'cancelled' && existing.status === 'posted') {
            for (const d of (returnDetails || [])) await adjustSourceQtyReturned(tenantClient, d, -Number(d.qty), -1);
            await reverseNonsaleableStockMovements(tenantClient, req.params.id);
            await reverseNonsaleableReturnLedger(tenantClient, req.params.id);
        }

        await logAudit(tenantId, req.auth.userId, 'change_nonsaleable_return_status', 'purchase_nonsalable_return', req.params.id, { new_status: status, cancellation_reason });
        await logDocumentAudit(tenantClient, tenantId, 'purchase_nonsalable_return', req.params.id, 'status_change', req.auth.userId, [{ field_key: 'status', new_value: status }]);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
router.get('/purchase-nonsaleable-returns/:id/audit-trail', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient
            .from('document_audit_trail')
            .select('*, performer:performed_by(full_name)')
            .eq('tenant_id', req.auth.tenantId).eq('document_type', 'purchase_nonsalable_return').eq('document_id', req.params.id)
...
router.delete('/purchase-nonsaleable-returns/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'delete'), async (req, res) => {
    try {
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const { data: existing } = await tenantClient.from('purchase_nonsaleable_returns').select('*').eq('id', req.params.id).eq('tenant_id', tenantId).single();
        ...
        await tenantClient.from('document_line_billing_terms').delete().eq('document_type', 'purchase_nonsalable_return').eq('document_id', req.params.id);
        await tenantClient.from('document_billing_terms').delete().eq('document_type', 'purchase_nonsalable_return').eq('document_id', req.params.id);
        const { error } = await tenantClient.from('purchase_nonsaleable_returns').delete().eq('id', req.params.id).eq('tenant_id', tenantId);
        if (error) throw error;
        await logAudit(tenantId, req.auth.userId, 'delete_draft_nonsaleable_return', 'purchase_nonsalable_return', req.params.id, { old_data: existing });
