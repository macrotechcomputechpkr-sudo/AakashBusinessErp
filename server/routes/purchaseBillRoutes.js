...
// Full CRUD for Purchase Bill - universally pulls forward
...
// FEATURE: "a Bill NOT sourced from a GRN never wrote to stock either"
// - a direct bill (no prior GRN) is the ONLY other way goods enter
// stock via Purchase, so it needs the SAME IN movement GRN now writes.
...
async function captureMasterSnapshots(tenantClient, b) {
    const lookups = [
        ...
        b.customs_office_id && tenantClient.from('customs_offices').select('office_name').eq('id', b.customs_office_id).maybeSingle()
    ];
    const [vendor, agent, warehouse, goodsAccount, goodsSubLedger, costCenter, businessUnit, area, route, customsOffice] = await Promise.all(
    ...
    return {
        ...
        customs_office_name_snapshot: customsOffice.data?.office_name || null
    };
}
...
// doc_no captured directly on each Bill line. Bill's own sources are
// GRN, Order, and/or Quotation (NOT Requisition - that's too far back
// in the chain to bill against directly).
async function captureSourceDocNo(tenantClient, d) {
    if (d.source_grn_detail_id) {
        const { data } = await tenantClient.from('purchase_grn_details').select('grn_id').eq('id', d.source_grn_detail_id).maybeSingle();
        if (data) {
            const { data: grn } = await tenantClient.from('purchase_grns').select('doc_no').eq('id', data.grn_id).maybeSingle();
            if (grn) return grn.doc_no;
        }
    }
    if (d.source_order_detail_id) {
...
async function syncDetails(tenantClient, tenantId, billId, details) {
    await tenantClient.from('purchase_bill_details').delete().eq('bill_id', billId);
    ...
    const rows = await Promise.all(details.map(async (d, i) => {
        ...
        return {
            ...
            item_import_taxable_amount: d.item_import_taxable_amount !== '' && d.item_import_taxable_amount !== undefined ? d.item_import_taxable_amount : null,
            item_import_tax_free_amount: d.item_import_tax_free_amount !== '' && d.item_import_tax_free_amount !== undefined ? d.item_import_tax_free_amount : null,
            ...snapshots
        };
    }));
    const { data: insertedRows, error } = await tenantClient.from('purchase_bill_details').insert(rows).select('id, display_order').order('display_order');
    if (error) throw error;
...
// every line into ready-to-use Bill detail rows, carrying the source
// ids forward so the source GRN can later track billed-vs-received progress.
...
router.get('/purchase-bills/pull-forward', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const { quotation_id, order_id, grn_id } = req.query;
        if (!quotation_id && !order_id && !grn_id) {
            return res.status(400).json({ success: false, error: 'Provide a quotation_id, order_id, and/or grn_id to pull from' });
        }
        ...
        if (order_id) {
            ...
            if (ord) {
                ...
                // Billing an Order DIRECTLY (no GRN in between - e.g. a
                // service purchase) bills the full ordered qty, not an
                // "outstanding vs received" figure - that tracking is
                // specifically for the GRN stage, not this one.
                details = details.concat((ord.details || []).map(d => ({
                    source_order_detail_id: d.id, source_doc_no: ord.doc_no, product_id: d.product_id, qty: d.qty, uom_id: d.uom_id,
                    rate: d.rate, warehouse_id: d.warehouse_id, barcode: d.barcode
                })));
            }
        }
        if (grn_id) {
            const { data: grn } = await tenantClient.from('purchase_grns').select('*, details:purchase_grn_details(*)').eq('id', grn_id).single();
            if (grn) {
                master = { ...master, vendor_ledger_id: master.vendor_ledger_id || grn.vendor_ledger_id, vendor_sub_ledger_id: master.vendor_sub_ledger_id || ((master.vendor_ledger_id || grn.vendor_ledger_id) === grn.vendor_ledger_id ? grn.vendor_sub_ledger_id : null) || null, product_company_id: master.product_company_id || grn.product_company_id || null, agent_id: master.agent_id || grn.agent_id, warehouse_id: master.warehouse_id || grn.warehouse_id, cost_center_id: master.cost_center_id || grn.cost_center_id, business_unit_id: master.business_unit_id || grn.business_unit_id, area_id: master.area_id || grn.area_id, route_id: master.route_id || grn.route_id, currency: master.currency || grn.currency };
                // FEATURE: outstanding-aware - only pull what this GRN
                // hasn't already been billed for (qty minus qty_billed),
                // so re-pulling the same GRN twice never double-bills it.
                details = details.concat((grn.details || [])
                    .map(d => ({ ...d, outstanding: Math.max(0, Number(d.qty) - Number(d.qty_billed || 0)), alt_outstanding: Math.max(0, Number(d.alt_qty || 0) - Number(d.alt_qty_billed || 0)) }))
...
router.get('/purchase-bills', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        await ensureDefaultFieldControls(tenantClient, req.auth.tenantId, 'purchase_bill');
...
router.get('/purchase-bills/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient
            .from('purchase_bills')
        ...
        if (error) return res.status(404).json({ success: false, error: 'Purchase Bill not found' });

        const { data: appliedTerms } = await tenantClient
            .from('document_billing_terms').select('billing_term_id').eq('document_type', 'purchase_bill').eq('document_id', req.params.id);
        data.billing_term_ids = (appliedTerms || []).map(t => t.billing_term_id);

        const { data: lineTerms } = await tenantClient
            .from('document_line_billing_terms').select('detail_id, billing_term_id').eq('document_type', 'purchase_bill').eq('document_id', req.params.id);
        if (data.details) {
...
router.post('/purchase-bills', requireAuth, loadUserPermissions, requirePermission('ledger', 'create'), async (req, res) => {
    try {
        ...
        const fieldError = await checkCompulsoryFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'purchase_bill', req.body, isDraft);
        ...
        if (!docNo) {
            const { data: codeRow, error: codeErr } = await tenantClient.rpc('next_master_code', { seq_name: 'tenant_master.seq_purchase_bill_code', type_prefix: 'PBIL' });
        ...
        const { data: doc, error } = await tenantClient
            ...
            .insert({
                ...
                import_detail_mode: b.import_detail_mode || null,
                customs_office_id: b.customs_office_id || null,
                customs_declaration_no: b.customs_declaration_no || null, customs_declaration_date: b.customs_declaration_date || null,
                bill_wise_import_taxable_amount: b.bill_wise_import_taxable_amount || null,
                bill_wise_import_tax_free_amount: b.bill_wise_import_tax_free_amount || null,
        ...
        try {
            ...
            const documentAdjustment = await syncBillingTerms(tenantClient, tenantId, 'purchase_bill', doc.id, b.billing_term_ids, total, totalQty);
            const lineTermResult = await syncLineBillingTerms(tenantClient, tenantId, 'purchase_bill', doc.id, detailsToSave, detailIdByIndex, b.summary_overrides);
            await applyTermSubLedgers(tenantClient, tenantId, 'purchase_bill', doc.id, b.term_sub_ledgers);
            await tenantClient.from('purchase_bills').update({ total_amount: total + documentAdjustment + lineTermResult.total }).eq('id', doc.id);
        } catch (syncErr) {
            await tenantClient.from('document_line_billing_terms').delete().eq('document_type', 'purchase_bill').eq('document_id', doc.id);
            await tenantClient.from('document_billing_terms').delete().eq('document_type', 'purchase_bill').eq('document_id', doc.id);
            await tenantClient.from('purchase_bills').delete().eq('id', doc.id);
            return res.status(400).json({ success: false, error: syncErr.message || 'Could not save line items' });
        }

        await logAudit(tenantId, req.auth.userId, 'create_purchase_bill', 'purchase_bill', doc.id, { doc_no: doc.doc_no });
        await logDocumentAudit(tenantClient, tenantId, 'purchase_bill', doc.id, 'create', req.auth.userId);
        res.json({ success: true, message: `Bill ${doc.doc_no} created`, data: doc });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

router.put('/purchase-bills/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'edit'), async (req, res) => {
    try {
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const b = req.body;
        const { data: existing } = await tenantClient.from('purchase_bills').select('*').eq('id', req.params.id).eq('tenant_id', tenantId).single();
        ...
        await lockProtectedFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'purchase_bill', b, existing);
        ...
        const fieldError = await checkCompulsoryFields(await getTenantClient(req.auth.tenantId), req.auth.tenantId, req.auth.userId, 'purchase_bill', b, !!(b.save_as_draft || b.status === 'draft'));
        ...
        if (b.details) {
            ...
            const documentAdjustment = await syncBillingTerms(tenantClient, tenantId, 'purchase_bill', req.params.id, b.billing_term_ids, total, totalQty);
            const lineTermResult = await syncLineBillingTerms(tenantClient, tenantId, 'purchase_bill', req.params.id, b.details, detailIdByIndex, b.summary_overrides);
            await applyTermSubLedgers(tenantClient, tenantId, 'purchase_bill', req.params.id, b.term_sub_ledgers);
            await tenantClient.from('purchase_bills').update({ total_amount: total + documentAdjustment + lineTermResult.total }).eq('id', req.params.id);
        }
        ...
        await logDocumentAudit(tenantClient, tenantId, 'purchase_bill', req.params.id, 'update', req.auth.userId, changes);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
// FEATURE: keeps the source GRN's own qty_billed current whenever a
// Bill against it is posted (delta > 0) or a posted Bill against it is
// cancelled (delta < 0, giving the qty back), and rolls the GRN's own
// status forward/backward to match - so a later pull-forward always
// offers exactly what's genuinely still outstanding.
async function adjustGrnQtyBilled(tenantClient, sourceGrnDetailId, delta, altDelta = 0) {
    const { data: srcLine } = await tenantClient.from('purchase_grn_details').select('qty_billed, grn_id, qty').eq('id', sourceGrnDetailId).maybeSingle();
    if (!srcLine) return;
    const newQtyBilled = Math.max(0, Number(srcLine.qty_billed || 0) + delta);
    await tenantClient.from('purchase_grn_details').update({ qty_billed: newQtyBilled }).eq('id', sourceGrnDetailId);
    await bumpAltCounter(tenantClient, 'purchase_grn_details', sourceGrnDetailId, 'alt_qty_billed', altDelta);

    // GRN status in BASE units (loose pieces count): billed / partially / received.
    await rollHeaderStatus(tenantClient, { headerTable: 'purchase_grns', detailTable: 'purchase_grn_details', fk: 'grn_id', headerId: srcLine.grn_id,
        counter: 'qty_billed', altCounter: 'alt_qty_billed',
...
router.put('/purchase-bills/:id/status', requireAuth, loadUserPermissions, requirePermission('ledger', 'edit'), async (req, res) => {
    try {
        ...
        // FEATURE: qty_billed only ever moves at the POSTED transition (a
        // draft bill hasn't really committed anything yet) - and only
        // ONCE, guarded by existing.status !== 'posted' so re-posting an
        // already-posted bill (a no-op status change) can never double-
        // count. Cancelling a POSTED bill gives the qty back.
        const { data: billDetails } = await tenantClient.from('purchase_bill_details').select('*').eq('bill_id', req.params.id);
        if (status === 'posted' && existing.status !== 'posted') {
            ...
            // FEATURE: "Challan Effect On Stock and Account" continued -
            // a Bill sourced from a GRN clears that GRN's provisional
            ...
            // Bill is a 'cr' reference (it increases what we owe). If
            // the vendor has bill-wise tracking on, use whatever
            // settlement breakdown the frontend confirmed (its own FIFO
            // suggestion, possibly hand-adjusted) to first clear any
            // outstanding Dr balance before the remainder floats as this
            // Bill's own new outstanding.
            if (data.vendor_ledger_id) {
                ...
                if (bwEnabled) {
                    ...
                    if (!settlements) {
                        const outstanding = await getOutstandingReferences(tenantClient, data.vendor_ledger_id, 'dr', data.product_company_id || null);
                    ...
                    await createReferenceAndSettle(tenantClient, tenantId, { productCompanyId: data.product_company_id || null,
                        ledgerId: data.vendor_ledger_id, sourceType: 'purchase_bill', sourceId: data.id,
        ...
        await logAudit(tenantId, req.auth.userId, 'change_bill_status', 'purchase_bill', req.params.id, { new_status: status, cancellation_reason });
        await logDocumentAudit(tenantClient, tenantId, 'purchase_bill', req.params.id, 'status_change', req.auth.userId, [{ field_key: 'status', new_value: status }]);
        res.json({ success: true, data });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
router.get('/purchase-bills/:id/audit-trail', requireAuth, loadUserPermissions, requirePermission('ledger', 'view'), async (req, res) => {
    try {
        const tenantClient = await getTenantClient(req.auth.tenantId);
        const { data, error } = await tenantClient
            .from('document_audit_trail')
            .select('*, performer:performed_by(full_name)')
            .eq('tenant_id', req.auth.tenantId).eq('document_type', 'purchase_bill').eq('document_id', req.params.id)
...
router.delete('/purchase-bills/:id', requireAuth, loadUserPermissions, requirePermission('ledger', 'delete'), async (req, res) => {
    try {
        const tenantId = req.auth.tenantId;
        const tenantClient = await getTenantClient(tenantId);
        const { data: existing } = await tenantClient.from('purchase_bills').select('*').eq('id', req.params.id).eq('tenant_id', tenantId).single();
        ...
        if (existing.status !== 'draft') {
            return res.status(400).json({ success: false, error: 'Only a Draft can be deleted - use Cancel for a posted bill' });
        }
        await tenantClient.from('document_line_billing_terms').delete().eq('document_type', 'purchase_bill').eq('document_id', req.params.id);
        await tenantClient.from('document_billing_terms').delete().eq('document_type', 'purchase_bill').eq('document_id', req.params.id);
        const { error } = await tenantClient.from('purchase_bills').delete().eq('id', req.params.id).eq('tenant_id', tenantId);
        if (error) throw error;
        await logAudit(tenantId, req.auth.userId, 'delete_draft_purchase_bill', 'purchase_bill', req.params.id, { old_data: existing });
